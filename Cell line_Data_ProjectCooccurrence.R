...
inputData <- read.table(file = paste0(rawdatafolder, "\\alterations_across_cell lines.tsv"),
                        header = TRUE,
                        sep = "\t")
...
drawPie(pGene = "KRAS", nudgeX = 0.75, title = "KRAS", direction = 1, angle = 0.3)
dev.off()
...
drawPie(pGene = "EGFR", nudgeX = 0.75, title = "EGFR", direction = 1, angle = 0.09)
dev.off()
...
write.table(x = pp_inputData,
            file = paste0(fileresultfolder, "\\Cell line Data Class assignment and tuples.txt"),
            row.names = FALSE,
            col.names = TRUE,
            sep = "\t",
            quote = FALSE)
